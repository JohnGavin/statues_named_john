...
> A count of the bronzes revealed,
> That John's common form was unsealed.
> While feminine grace,
> Held a much smaller place,
> A sad truth the numbers unpeeled.
...
top_names <- gender_analysis$top_names_by_gender %>%
  dplyr::filter(inferred_gender %in% c("Female", "Male")) %>%
  dplyr::select(Gender = inferred_gender, Name = extracted_names, Count = n, Percent = percent) %>%
  dplyr::arrange(Gender, desc(Count))

knitr::kable(top_names, caption = "Most Common First Names by Gender")
```
