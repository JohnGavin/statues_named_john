...
# Absolute store path: Quarto may execute this chunk from inst/qmd/, where a
# relative "_targets" does not exist.
store_path <- file.path(rprojroot::find_root(rprojroot::is_r_package), "_targets")
if (dir.exists(store_path)) {
  all_memorials <- targets::tar_read(all_memorials, store = store_path)
  summary_table <- targets::tar_read(summary_table, store = store_path)
  findings <- targets::tar_read(findings, store = store_path)
  category_plot <- targets::tar_read(category_plot, store = store_path)
  memorial_map_plot <- targets::tar_read(memorial_map_plot, store = store_path)
  johns_comparison <- targets::tar_read(johns_comparison, store = store_path)
  memorial_interactive_map <- targets::tar_read(memorial_interactive_map, store = store_path)

  gender_analysis <- targets::tar_read(gender_analysis, store = store_path)
} else {
